...
export default {
  ...
  methods: {
    async onSaveRole () {
      ...
      if (!this.model.id) {
        try {
          const create_payload = this.$model.CREATE_ROLE_PAYLOAD(this.model)
          const { message, status, data } = (await this.$request.postData(this.$url.ROLE_URL, create_payload, this.jwt))
          if (status === 1) {
        ...
        try {
          const update_payload = this.$model.CREATE_ROLE_PAYLOAD(this.model)
          ...
          if (status === 1) {
            ...
            successMessage = `Success: Role "${this.model.name}"  was Updated!.`
