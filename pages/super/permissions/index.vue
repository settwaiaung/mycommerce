  ...
  <div>
    <v-stepper v-model="step" elevation="0">
      <v-stepper-items>
        ...
        <PermissionForm v-model="selectedPermission" @onGoBack="handleGoBack" />
      </v-stepper-items>
    </v-stepper>
...
import PermissionForm from '@/components/forms/super/PermissionForm.vue'
export default {
  components: {
    PermissionForm
  },
  ...
  mounted () {
    ...
    this.$store.dispatch('setTitle', 'Permissions')
    ...
    this.$on('deletedData', (data) => {
      ...
      this.permissions.splice(foundIndex, 1)
    })
  },
  methods: {
    ...
    onEdit (item) {
      this.selectedPermission = (({ id, name }) => ({
        id,
        name
    ...
    onDelete (item) {
      this.$emit('deleteForm', item, this.$url.PERMISSION_URL, this.jwt)
    }
  }
}
</script>
